...
class GenerateLibs extends DefaultTask {
    ...
    private final boolean forLinuxArm32 = buildEnvs?.contains('linuxarm')
    ...
    void generate() {
        ...
        if (forLinuxArm64) {
            def linuxArm64 = BuildTarget.newDefaultTarget(BuildTarget.TargetOs.Linux, true, true)
            addFreeTypeIfEnabled(linuxArm64)
        ...
        if (forLinuxArm32)
            BuildExecutor.executeAnt(jniDir + '/build-linuxarm.xml', commonParams)
        if (forMac)
